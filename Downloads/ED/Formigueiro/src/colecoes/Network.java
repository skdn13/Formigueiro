...
public class Network<T> extends Graph<T> implements NetworkADT<T> {
    ...
    public Network() {
        numVertices = 0;
        this.adjMatrix = new ITunel[DEFAULT_CAPACITY][DEFAULT_CAPACITY];
    ...
    public String toString() {
        ...
        for (int i = 0; i < numVertices; i++) {
            ...
            for (int j = 0; j < numVertices; j++) {
                if (adjMatrix[i][j].getDistance() < Double.POSITIVE_INFINITY) {
                    result += "1 ";
                } else {
                    result += "0 ";
                }
    ...
    public void addEdge(T t, T t1, ITunel itunel) throws ElementNotFoundException {
        addEdge(getIndex(t), getIndex(t1), itunel);
    }
    ...
    public void removeEdge(int index1, int index2) {
        if (indexIsValid(index1) && indexIsValid(index2)) {
            adjMatrix[index1][index2].setDistance((int) Double.POSITIVE_INFINITY);
            adjMatrix[index2][index1].setDistance((int) Double.POSITIVE_INFINITY);
        }
    }
    ...
    public void addVertex() {
        ...
        for (int i = 0; i <= numVertices; i++) {
            adjMatrix[numVertices][i].setDistance((int) Double.POSITIVE_INFINITY);
            adjMatrix[i][numVertices].setDistance((int) Double.POSITIVE_INFINITY);
        }
        numVertices++;
    }
    ...
    protected Iterator<Integer> iteratorShortestPathIndices(int startIndex, int targetIndex) {
        ...
        do {
            ...
            for (int i = 0; i < numVertices; i++) {
                if (!visited[i]) {
                    if ((adjMatrix[index][i].getDistance() < Double.POSITIVE_INFINITY)
    ...
    public Network mstNetwork() {
        ...
        resultGraph.adjMatrix = new ITunel[numVertices][numVertices];
        for (int i = 0; i < numVertices; i++) {
            for (int j = 0; j < numVertices; j++) {
    ...
    protected void expandCapacity() {
        ...
        ITunel[][] largerAdjMatrix
                = new ITunel[vertices.length * 2][vertices.length * 2];
